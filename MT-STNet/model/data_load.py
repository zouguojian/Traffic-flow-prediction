...
def seq2instance(data, P, Q, low_index=0, high_index=100, granularity=15, sites = 108, type='train'):
    ...
    while low_index + P + Q < high_index:
        label = data[(low_index + P) * sites: (low_index + P + Q) * sites, -1:]
        label = np.concatenate([label[i * sites: (i + 1) * sites] for i in range(Q)], axis=1)
        ...
        L.append(np.reshape(label,[1, sites, Q]))
