...
from lib.utils import load_data_new, get_adjacency_matrix, evaluate_on_test_mstgcn, compute_val_loss_mstgcn, predict_and_save_results_mstgcn, load_adjacent
# from torch.utils.tensorboard import SummaryW/riter

parser = argparse.ArgumentParser()
parser.add_argument("--config", default='configurations/YINCHUAN.conf', type=str,
...
DEVICE = torch.device('cuda:' + str(ctx) if torch.cuda.is_available() else "cpu")
...
net = make_model(DEVICE, nb_block, in_channels, K, nb_chev_filter, nb_time_filter, time_strides, adj_mx,
                 num_for_predict, len_input, _mean, _std)
...
def train_main():
    ...
    for epoch in range(start_epoch, epochs):
        ...
        params_filename = os.path.join(params_path, 'yinchuan')
        # evaluate_on_test_mstgcn(net, test_loader, test_target_tensor,epoch, _mean, _std)
        val_loss = compute_val_loss_mstgcn(net, val_loader, epoch)
        ...
        for batch_index, batch_data in enumerate(train_loader):
            ...
            if global_step % 100 == 0:
                ...
                print('global step: %s, training loss: %.2f' % (global_step, training_loss))
