...
def compute_val_loss_mstgcn(net, val_loader, epoch, limit=None):
    '''
...
def evaluate_on_test_mstgcn(net, test_loader, test_target_tensor, epoch, _mean, _std):
    '''
...
def predict_and_save_results_mstgcn(net, data_loader, data_target_tensor, global_step, _mean, _std, params_path, type):
    ...
    with torch.no_grad():
        ...
        np.savez_compressed('data/MSTGCN-' + 'YINCHUAN', **{'prediction': prediction, 'truth': data_target_tensor})
        ...
        for (l,r) in [(0,66)]:
            for i in range(prediction_length):
                ...
                mae, rmse, mape = metric(prediction[:, l:r, i], data_target_tensor[:, l:r, i])
            ...
            mae, rmse, mape = metric(prediction[:, l:r], data_target_tensor[:, l:r])
