...
class Trainer(object):
    ...
    def train_epoch(self, epoch, iteration, start_time):
        self.model.train()
        total_loss = 0
    ...
    def train(self):
        ...
        for epoch in range(1, self.args.epochs + 1):
            ...
            train_epoch_loss, iteration = self.train_epoch(epoch, iteration, start_time)
        ...
        self.logger.info("The best loss: {:.6f}".format(best_loss))
    ...
    def test(model, args, data_loader, scaler, logger, path=None):
        ...
        # else:
        #     y_pred = scaler.inverse_transform(torch.cat(y_pred, dim=0))
        np.savez_compressed('data/AGCRN-' + 'YINCHUAN', **{'prediction': y_pred.cpu().numpy(), 'truth': y_true.cpu().numpy()})
        # np.save('./{}_true.npy'.format(args.dataset), y_true.cpu().numpy())
        # np.save('./{}_pred.npy'.format(args.dataset), y_pred.cpu().numpy())
        # print(y_pred.shape)
        # torch.Size([5295, 12, 66, 1])
        print('                MAE\t\tRMSE\t\tMAPE')
        for (l,r) in [(0,13),(13,26),(26,66)]:
            for t in range(y_true.shape[1]):
                mae, rmse, mape, _, _ = All_Metrics(y_pred[:, t, l:r], y_true[:, t, l:r],
                                                    args.mae_thresh, args.mape_thresh)
                print('step: %02d         %.3f\t\t%.3f\t\t%.3f%%' % (t + 1, mae, rmse, mape * 100))
                # logger.info("Horizon {:02d}, MAE: {:.2f}, RMSE: {:.2f}, MAPE: {:.4f}%".format(
                #     t + 1, mae, rmse, mape*100))
            mae, rmse, mape, _, _ = All_Metrics(y_pred[:,:,l:r], y_true[:,:,l:r], args.mae_thresh, args.mape_thresh)
