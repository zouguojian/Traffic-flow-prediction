...
class DCRNNSupervisor:
    def __init__(self, data_type, LOAD_INITIAL, adj_mx, **kwargs):
        ...
        total_num = sum(p.numel() for p in dcrnn_model.parameters())
        trainable_num = sum(p.numel() for p in dcrnn_model.parameters() if p.requires_grad)
    ...
    def _get_log_dir(kwargs):
        ...
        if log_dir is None:
            ...
            log_dir = os.path.join(base_dir, 'dcrnn/')
    ...
    def save_model(self, epoch):
        path = 'data/model/pretrained/YINCHUAN/%s_best' % self.data_type
        if not os.path.exists('data/model/pretrained/YINCHUAN/'):
            os.makedirs('data/model/pretrained/YINCHUAN/')
    ...
    def evaluate(self, dataset='val', batches_seen=0):
        ...
        with torch.no_grad():
            ...
            for t in range(y_preds.shape[0]):
                ...
                # mae, mape, rmse = metric(y_pred, y_truth)
                # print('step: %02d         %.3f\t\t%.3f\t\t%.3f%%' % (t + 1, mae, rmse, mape * 100))
                y_truths_scaled.append(y_truth)
                y_preds_scaled.append(y_pred)
            # mae, mape, rmse = metric(np.concatenate(y_preds_scaled,axis=0),np.concatenate(y_truths_scaled,axis=0))
        ...
        with torch.no_grad():
            ...
            for (l,r) in [(0,66)]:
                for t in range(y_preds.shape[0]):
                    ...
                    metrics = self._compute_metrics(y_truths[t,:,l:r], y_preds[t,:,l:r])
                    log = 'Evaluate best model on test data for horizon {:d}, Test MAE: {:.4f}, Test MAPE: {:.4f}, Test RMSE: {:.4f}'
                    print('step: %02d         %.3f\t\t%.3f\t\t%.3f%%' % (t + 1, metrics[0], metrics[2], metrics[1] * 100))
                    # print(log.format(t + 1, metrics[0], metrics[1], metrics[2]))
                metrics = self._compute_metrics(y_truths[:,:,l:r], y_preds[:,:,l:r])
                print('average:         %.3f\t\t%.3f\t\t%.3f%%' %(metrics[0], metrics[2], metrics[1] * 100))
                print('\n')
        return metrics[0], {'prediction': y_preds, 'truth': y_truths}
    ...
    def _train(self, base_lr,
        ...
        for epoch_num in range(self._epoch_num, epochs):
            ...
            if (epoch_num % log_every) == log_every - 1:
                message = 'Epoch [{}/{}] ({}) train_mae: {:.4f}, val_mae: {:.4f}, lr: {:.6f}'.format(epoch_num, epochs, batches_seen,
