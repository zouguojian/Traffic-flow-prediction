...
parser.add_argument('--N', type = float, default = 66,
                    help = 'the node number in graph')
...
parser.add_argument('--model_file', default = 'experiments/',
                    help = 'save the model to disk')
parser.add_argument('--log_file', default = 'log(PEMS)',
                    help = 'log file')
parser.add_argument('--adjacent_file', default = 'data/sensor_graph/adjacent.npz',
                    help = 'adjacent file')
...
if args.name == 'YINCHUAN':
    adj = load_graph_adj(args.adjacent_file).astype(np.float32)
else: adj = load_graph_data(args.adjacent_file)
...
def res(model, valX, valTE, valY, mean, std):
    ...
    np.savez_compressed('data/ST-GRAT-' + 'YINCHUAN', **{'prediction': pred, 'truth': label})
...
def train(model, trainX, trainTE, trainY, valX, valTE, valY, mean, std):
    if not os.path.exists(args.model_file):
        os.makedirs(args.model_file)
        print('create params directory %s' % (args.model_file))
    ...
    for epoch in tqdm(range(1,args.max_epoch+1)):
        ...
        if mae[-1] < min_loss:
            min_loss = mae[-1]
            torch.save(model, args.model_file + args.name)

def test(model, valX, valTE, valY, mean, std):
    # torch.load(args.model_file + args.name)
    model = torch.load(args.model_file + args.name, map_location = device)
    mae, rmse, mape = res(model, valX, valTE, valY, mean, std)
