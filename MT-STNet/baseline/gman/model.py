...
def placeholder(P, Q, N):
    X = tf.placeholder(shape=(None, P, N), dtype=tf.float32)
    TE = tf.placeholder(shape=(None, P + Q, 2), dtype=tf.int32)
    label = tf.placeholder(shape=(None, Q, N), dtype=tf.float32)
...
def GMAN(X, TE, SE, P, Q, T, L, K, d, bn, bn_decay, is_training):
    ...
    STE_Q = STE[:, P:]
    # encoder
    X = tf.add(X, STE_P) # 和我们的一致
    for _ in range(L):
        X = STAttBlock(X, STE_P, K, d, bn, bn_decay, is_training)
    print('the encoder out shape is : ',X.shape)
    # transAtt
    ...
    X = tf.transpose(X, [0, 2, 1], name='output_y')
    return X